...
public class ResurrectedZone : EntranceChecker
{
    ...
    private void TryToResurrect(Unit unit)
    {
        if (_allDetectedEnemy.Contains(unit) && _resurrectChance >= Random.Range(0, 100) && !unit.IsResurrected)
        {
            ...
            spawnedZombie.SetTargetAltar(_unit.TargetEnemyAltar);
            ...
            spawnedZombie.GetComponent<ZombieStateMachine>().Init();
            spawnedZombie.GetComponent<ZombieStateMachine>().SerTargetAltar(_unit.TargetEnemyAltar);
